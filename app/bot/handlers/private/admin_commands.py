...
async def _send_banned_users(manager: Manager, redis: RedisStorage) -> None:
    ...
    for i, user_data in enumerate(banned_users):
        ...
        builder.button(
            text=f"Разбанить {user_data.full_name}",
            callback_data=f"unban_user_{user_data.id}",
        )
...
async def show_banned_users_callback(
    call: CallbackQuery, manager: Manager, redis: RedisStorage
) -> None:
    await _send_banned_users(manager, redis)
    await call.answer()
...
async def unban_user_callback(
    ...
    await call.answer(
        f"Пользователь {hbold(user_data.full_name)} (ID: {user_id}) разбанен."
    )
    await _send_banned_users(manager, redis)
...
async def unban_user_command(
    ...
    if not user_data.is_banned:
        await message.reply(
            f"Пользователь {hbold(user_data.full_name)} (ID: {user_id}) не забанен."
        )
        return
